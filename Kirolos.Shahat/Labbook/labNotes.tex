...
\labday{Tuesday, August 1 2017}
\experiment{Goals for today}
\begin{itemize}
	\item Continue fixing errors
\end{itemize}

\experiment{Current Progress and Notes}
\begin{itemize}
	\item Ended up having to revert to an old commit on my personal fork of the repo and was having issues with that. Finally figured it out and then was having issues with mergetool. Finally fixed both issues.
	\item Goal for next time is to update my fork with Cedric's repo and merge the proper changes and then continue with figuring out the error I was getting previously.
\end{itemize}


%-----------------------------------------

\labday{Tuesday, August 8 2017}
\experiment{Goals for today}
\begin{itemize}
	\item Begin documentation and prepare for final merge. I've been struggling with git for about a week but it was a simple fix but since my last day is friday then I'll just try to document as much as I can before I leave.
\end{itemize}

\experiment{Current Progress and Notes}
\begin{itemize}
	\item Did some slight optimizations, commenting, and fixed a small bug in FONSEParameter.cpp
	\item Goal for next time: Look at FONSEModel.cpp
\end{itemize}
%----------------------------------------------------------------------------------------
