...
\begin{description}
  \item[Project State:]{
    \begin{itemize}
    \item{ Currently have a function that updates the grantham distance matrix/hash table for every new alpha beta and gamma weight. Using alpha beta and gamma from the paper I can recreate the Grantham distance matrix between the amino acids displayed in the paper for all but one pair of amino acids: Tryptophan and Aspartic Acid. I generate a value of 191 but the paper lists a value of 181. I need to look into this. Otherwise, the script is ready to be turned into a nested likelihood optimization.  }
    \end{itemize}
    
  \item[Current Tasks:] Next time I need to figure out why there is a discrepency between my matrix and the paper's matrix between tryptophan and aspartic acid. I also need to finish implementing my nested optimization loop.
  \item[Future Tasks:]
  \begin{itemize}
  \item{streamline optimization so it is faster (pre-generate all values): $X$}
  \item{create a a function that can easily generate toy subsets of sequences that will allow me to verify that my code works.}
   landscape}
  \item{I need create outer loop for testing different weighting parameters for the distances: Next}
  \item{Need to verify my optimization schema works by testing it on the toy sequences}
  \item{Identify which optimization algorithm works best on toy sequences}
  \item{implement a basin-hopping algorithm to verify we have reached the real local optimum}
  \item{test optimization schema on real subset of the gag sequences}
...
\begin{description}
  \item[Project State:]{
    \begin{itemize}
    \item{Fixed hanging issue, and created framework to integrate immune fits into dictionary. Reworked a function allowing me to quickly consensus useful parts of the dictionary into a csv file.} 
    \end{itemize}
  \item[Current Tasks:] I need to integrate my immune freq fits into the hash table and regenerate my fitness landscape.
  
  \item[Future Tasks:]
  \begin{itemize}
  
  \item{Need to re-run immune fits by type}
  \item{Need to address and flag parameters that have gone out of bounds}
  \item{I need to integrate my immune freq fits into the hash table and regenerate my fitness landscape}
  \item{need to write likelihood ratio test routine to accept/reject immune model fits}
  \end{itemize}
  \end{description}
   
  

  \labday{September 26th }
%=======================================================================================

  
 \experiment{Phys-Chem}
\researcher{}
\begin{description}
  \item[Project State:]{
    \begin{itemize}
    \item{I have fixed the divide by zero error in the optimization. I do notice that the optimizer was getting stuck when the choice of weights
    for the physio-chemical properties generated a distance matrix with over-large distances. I need to rescale the grantham matrix for each run so the distances are on average 100 units. We can easily compare Gs by multiplying our estimated G by the particular scaling term for that run. Alternatively, we can simply conceptualize the scaling as being a bound on our weighting parameters. We always linearly scale our property weights so that we always have a distance matrix averaged around 100 units} 
    \end{itemize}
  \item[Current Tasks:] I need to edit the grantham matrix function so that the mean of the distances scale to 100. 
  
  \item[Future Tasks:]
  \begin{itemize}

  \item{Make sure likelihood scoring function for nested fit makes sense}
  \item{Make sure I understand how the Physics Models and the Population Genetics Models are related}
  \end{itemize}
  \end{description}
