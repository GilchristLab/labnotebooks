# THIS SCRIPT DOES NOT WORK NATIVELY IN LABNOTEBOOKS.
# THE .RDAT FILES ARE TOO LARGE TO INCLUDE IN OUR GIT REPO.
...
#    skip("RFP Model testing is optional.")
  ...
  restartFile = file.path("UnitTestingOut", folder, "restartFiles", "20000_restartJeremyFile.rst")
  MCMCObject = file.path("UnitTestingOut", folder, "MCMCJeremyObject.Rdat")
  ParameterObject = file.path("UnitTestingOut", folder, "RFPJeremyObject.Rdat")
  ...
  test_that("file exists: MCMCJeremyObject.Rdat", {
    expect_equal(file.exists(MCMCObject), T)
  })
  
  test_that("file exists: RFPJeremyObject", {
    expect_equal(file.exists(ParameterObject), T)
  })
  ...
  parameter <- loadParameterObject(ParameterObject)
  mcmc <- loadMCMCObject(MCMCObject)
  ...
  percentTraceIgnore <- 0.5
  
  start <- length(loglik.trace) * .7 #percentTraceIgnore 
