...
\begin {itemize}
    ...
    \item The source code itself, which requires Mathmatica (now downloading) is similarly found there.
...
% P_{i} | W_{i} proportional to (alphaesque) k_{g} little sigma _{i-1} omega{i} = k_{g}
...
\begin{itemize}
    ...
    \item I fixed the Rstudio issue wherein I could not have it load our libary; I need to set the working directory each time I use Rstudio on the server, since normally it would do this automatically by clicking on a file (but on the server, it just loads from my home directory).
...
\begin{itemize}
    ...
    \item Bug still occurred. It seems Travis cannot create .csv files at all (but can create .pdf and .txt files). Readded .csv files to finally fix the bug.
...
\begin{itemize}
    ...
    ** (evince:19638): WARNING **: Could not open X display
    ...
    \end{lstlisting}
    \item Meanwhile, for the script itself I tried to relabeled the plots and readded restart files.
...
\begin{itemize}
    ...
    \item Take his SimulatedGenome and do the same thing (so same process but ignore creatng a simulated genome) -- SimulatedRFPData.csv in RibModelDev.
    ...
    \begin{itemize}
        ...
        If you want to start your seach at those values -- how do you do that?
    ...
    \begin{itemize}
        ...
        \item We \textbf{do} have (probably) the true Phi values: under the rfp folder, we can acces RFPPhiValues.csv.
    \end{itemize}
...
\begin{itemize}
    ...
    \item Thus, I should try to extract this number as well as the the true Phi value and as points (x,y) and hope that a line will be drawn where x=y.
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        \center
        \includegraphics[width=\textwidth,keepaspectratio]{7-27-16-JeremyCorrelation.png}
        \label{Jeremy Correlation}
    \end{figure}
    \item The bottom axis, not shown (see the newly created folder \enquote{7-27-16-JeremyCorrelation}) is True Values.
    ...
    \item The .fasta file we will use is \enquote{s288c.genome.fasta}, currently found in Rib\sep Model\sep Dev\sep /\sep data\sep /\sep /
    \item The LambdaPrime, Alpha, and Phi values can be recycled from Jeremy's folder, however.
    \item I am going to use the data extracted from the 20,000 sample run 
\end{itemize}
