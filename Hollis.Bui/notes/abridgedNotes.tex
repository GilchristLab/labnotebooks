...
\title{Abridged Notes for Undergraduate Research Work}
\author{Hollis Bui}
...
\begin{itemize}
    \item a) data on gene 
    \begin{itemize}
        \item ab) to feed into PA
    \end{itemize}
    \item or b) PANSE
\end{itemize}

\experiment{Lareau Data}
Which file type should I be reading in? PA or Fasta?
...
In the course of running an PA Model, the following functions are called (and have yet to be
unit tested).
...
\begin{itemize}
    ...
    \end{itemize}
    \item initPAParameterSet -- PA exclusive
    ...
    \end{itemize}
    \item setParameter -- PA model exclusive
    \item mcmc.run -- MCMC function on PA(TODO later?)
\end{itemize}
...
\experiment{Unit Testing}
\begin{itemize}
    \item Wrote and tested a function for numAcceptForSynthesisRate.
...
For now, I am switching to unit testing PA-exclusive functions.

\begin{itemize}
    \item initPAParameterSet (in PAParameter.cpp)
    \begin{itemize}
    ...
    \end{itemize}
    \item setParameter (in PAModel.cpp) -- may be untestable, ignore for now
\end{itemize}

In the course of working on checking PAParameter, wrote up Unit Testing for all
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{itemize}
        \end{itemize}
        \item mcmc::run -- MCMC function on PA
    \end{itemize}
    \item Write up pseudo-code with PANSE itself to prepare for it
    \item Create and test a function for reading in Lareau material (low priority)
    \item Parallelization is after the initial PANSE stuff is implemented, very low priority
...
Ask about current/proposed CSP Parameter, and how (if) to unit test
...
subitems first: (NOTE: Testing with PAModel)
\begin{itemize}
    \item mcmc::run
    \begin{itemize}
        \item mcmc::varyInitialConditions
        \begin{itemize}
            \item model::proposeCodonSpecificParameter -- wrapper to PAparameter
            ...
            \item model::updateCodonSpecificParameter -- wrapper to PAparameter
        ...
        \item model::initTraces -- wrapper to PAparameter initAllTraces
        ...
        \begin{itemize}
            ...
            \item PAparameter::updateCodonSpecificParameterTrace
        \end{itemize}
        \item model::setLastInteration -- wrapper to parameter (done?)
        \item model::writeRestartFile -- wrapper to PAparameter writeEntireRestartFile
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item model::\sep adapt\sep Codon\sep Specific\sep Parameter\sep Proposal\sep Width --
        wraps PA\sep parameter. Uses traces.
        \item mcmc::acceptRejectHyperParameter
        \begin{itemize}
            \item model::calculateLogLikelihodRatioForHyperParameters
            \item model::updateHyperParameter -- parameter::updateStdDevSynthesisRate
...
Changed quicksort (simple implementation) to std::sort.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item PAparameter-\textgreater initAllTraces
        \begin{itemize}
            \item traces.initializePATrace (PA-only)
            \begin{itemize}
    ...
    wrapper to PA\sep parameter. Uses trace
    \item mcmc::acceptRejectHyperParameter
    \begin{itemize}
...
I think I will create a separate function for each type of file. Basic Parameter, PA, ROC, etc.
...
\experiment{R Debugging}
Continued working on fixing R documentation. I was able to reduce the warnings
...
\begin{itemize}
    \item Last night (June 26) spent about two hours fixing a bug with the overall project:
there was an error with testhat due to changing (fixing) Parameter.cpp setup.
    \item Plan for today is to begin unit testing with R-side models while also rereading PANSE
documentation and planning psuedocode.
    \item Found a bug in function to simplify code; fixed bug, started replacing this code
...
The bug mentioned was in R/parameterObject.R: two functions (getMixtureAssignmentEstimate and getExpressionEstimatesForMixture) did not apply the unlist function for a random of values, and it has been corrected now.}
    \item Fixed another bug with traces on the R-side whenever plotting is done.\footnote{mikeg: 06/30/16 -- Better than the above note, but could still use a little more detail.}
\footnote{Hollis: 07/05/16 -- Acknowledged. 
...
in run\sep Simulated\sep PA\sep Data\sep .\sep R in Rib\sep Model\sep Dev:
...
\footnote{Hollis: 07/01/16 -- This is the part of the code in runSimulatedPAData.R that causes an error.
...
Then fixed run\sep Simulated\sep PA\sep Data.R: invalid method occurs calling:
...
\begin{itemize}
    \item Found a operating system independent function for grabbing file locations.
...
more efficient implementation without separators.
    \item Started reading the Pop 2014 paper.
\end{itemize}

Talked with Dr. Gilchrist further --

\begin{itemize}
    \item Stopping work on the ROC model file I have been editing for now.
    ...
    \item The pausing time model (PA) does get the Lambda and Alpha values.
    ...
    (again, scripts from Dev are old and liable to bugs) make a working PA script.
    ...
    \item Note for future-proofing and convenience, symbolic links are broken when using SCP. Instead, must use rsync to preserve symbolic links, i.e. with the Pop folder.
\end{itemize}

\labday {July 1, 2016}

\begin{itemize}
    \item Switching to unit testing with testMCMCROC by Cedric request, as Alan had changed dynamic arrays to vectors and this will ease debugging.
    \item Note that I had tested them before; inconclusive results.
    \item While I encountered slowdown in an initialization step upon changing, Alan experienced outright NaN errors and program crashing on both C and R side code (and his tests the day before yielded 50\% better results in terms of speed).
    \item Unit Testing for MCMC: Run a ROC MCMC simulation on a seed, get and save those results,
and on future testthat runs see if the output matches (specifically the Loglikelihood). Runs in about 3 seconds for ROC.
    \item Minor edits to the R-side documentation files, ran document() again. 
    \item During this mess I commented to Dr. Gilchrist on my progress on writing the PA unit testing.
    \item Finished file, works currently by literally comparing output files. Future implementation idea: Implement it comparing averaged logPosterior vector versus hard-coded value.
\end{itemize}

\labday {July 5, 2016}

\begin{itemize}
    \item Trying to debug Travis failure with testMCMCROC.R.
    \item Changed test so it checks the last log likelihood versus a hardcoded value only, so it wasn't a file issue.
    \item Suspect it may be due to different seeds being produced.
\end{itemize}

\labday {July 6, 2016}

\begin{itemize}
    \item Further attempts to debug testMCMCROC.R on Travis failing, while locally it passed.
    \item Testing just the one file shows the log likelihood returns NaN on iteration 10.
    \item While brainstorming ways to fix, started writing Pop processing and testing.
    \begin{itemize}
...
\begin{itemize}
    ...
    \item In summary, it was caused by merge conflicts with Alan's work changing dynamic arrays to vectors (causing NaN bugs), which was not debugged before being accepted into the main repo -- pulling from the main repo had caused the bugs.
    \item My local machine had not pulled from the main, bugged repo, so it ran fine, but other sources had to be pulled.
...
\begin{itemize}
    ...
    \item The first issue was that it was trying to write (sink) to a file that did not exist -- removing output redirection let the package run locally and on Travis successfully.
    \item Does not work currently on Gauley due to (as discovered later) Gauley using a buggy version of R -- logLikelihood is calculated differently despite using the same seed. 
...
\begin{itemize}
    ...
    \item Discovered in the future this was an R-version related bug, and it is solved as of June 1, 2017.
    ...
    \item Began writing a script to convert the format of the file Dr. Gilchrist had processed into the same format as our readPA function demanded.
\end{itemize}

\labday {July 14, 2016}

\begin{itemize}
    \item Examining the R script I am now modifying more closely, it may be Gabriel's old script that he had written to test the PA model.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item The important graphs to "prove" that it is run correctly are the RFP\sep \_\sep CSP\sep \_\sep Values and the correlation between Pop and PA Model Pausing Time Rates.
    ...
    \item Fixed Cedric's bug with getGenomeForGeneIndices -- seems to have been written with completely wrong code! Simply adapted the C-side code for the R implementation.
\end{itemize}
...
\begin{itemize}
    ...
    \end{lstlisting}
    \item Debugged why Travis was failing again when I added the output directory to .gitignore.
    \item Travis cannot create .csv files at all (but can create .pdf and .txt files), so empty .csv files are added to fix this bug. On git, files cannot be written to a directory that does not exist, and git does not recognize empty directories.
...
\begin{itemize}
    ...
    \item Fit PAmodel and compare (c.f) estimated phi to true phi.
    ...
    \item Fit it with PA, and see if the initial thousand or so are comparable.
    ...
    \item Overall TODO:
    \begin{itemize}
        \item Run and fit PA SimulatedRFPData.csv (Jeremy's) naive.
        \item Simulate own genome, then run PA model naive.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item To implement getCSPEstimates for PA (since this is currently unimplemented except in ROC), would have to change at least a bit of the code, confirming that it was more difficult than a quick fix.
        \item PA does not concern itself with AA's directly, but can work with the group list via getGroupList instead.
        ...
        However, it is not currently extendible to PA.
        \item This led to the TODO note about copying this function and adapting it to PA, but now it may be simpler to make it generic as Dr. Gilchrist had suggested.
...
\begin{itemize}
    \item Progress was stalled by a bug compiling the package: Need to update NAMESPACE. Fixed by naively editing our current file. TODO: Make this more easily modifiable by allowing us to use roxygen2 rather than manual edits.
...
\begin{itemize}
    \item Talk with Dr. Gilchrist:
    \begin{itemize}
        \item R/initializeGenomeObject default is fasta.
        ...
        \item PA reading and writing RFP files may be completely wrong: We \textbf{need} the codon\_counts, but currently we do not preserve it.
        \item Recall Pop et al data (originally): Name, Length, Sum mRNA, Sum FP, FP per Codon, which is a mess.
        \item This data was simplified via Dr. Gilchrist's Mathematica script into the current PA format.
        \item PA: Format should be ID, RFP Count, Codon Count, Codon (current format).
        ...
        \item RFP Codon counts will take account of ambiguous or non-observable positions (designated by -1, discarded line).
        \item New fuction to be created: processPA (defualt for both PA/RFPROC and PANSE models).
        \item TODO LATER: SimulatePANSE: just for each position, make RFP count number.
        \item TODO: Later on do Process Pop (which would also call readFasta) to get the PA format or the PANSE format.
        ...
        \item Make RFP\_count data to have multiple categories possible.
        \item TODO: Make an integer defaulting to just calculating long category.
    \end{itemize}
...
\begin{itemize}
    \item August 23 -- Mostly a discussion day with Cedric on how to re-implement PAModel
    \begin{itemize}
...
\begin{itemize}
    \item Created and set new data structures.
...
\begin{itemize}
    \item I'm going to try and polish off the PA file reading and then finally push these changes to the main repository.
...
\begin{itemize}
    \item Alex fixed a segfault: getGenomeForGeneIndices takes a R-style vector (1-indexed) while it uses C++ code that is 0-indexed (artifact from me copying C-style code over, not checking after implementing). Updated indices.
    \item Confirmed that read\sep PA fully works putting in positions and RFP\sep \_\sep counts.
...
\begin{itemize}
    \item Fixed three failures with checking Unit Testing: result of renaming logLikelihoodTrace to logPosteriorTrace on only the R-side code (some of it) without editing the existing Unit Testing files nor the cpp files.
...
\begin{itemize}
    ...
    \item End of day result: Writing function 90\% completed. 
...
\labday{March 16 - 17, 2017}

\begin{itemize}
    \item Added Hmisc to travis.yml -- this fixes Hmisc on Travis.
    \item Added acf to imports for NAMESPACE.
    \item Writing function goal: need to record, for each position, the RFP values for each category. All other info is optional or already stored elsewhere.
\end{itemize}
...
\labday{May 18, 22, 23 -- 2017}

\begin{itemize}
    \item Fixed openMP flags on OSX (Mac).
    \item For Cran-compliancy, Cedric had added flags that resulted in out-of-box Mac installations to fail as it required openMP (worked on Windows, Linux, and CRAN itself as it worked on 2 of the 3 primary OS's).
    \item Now there's a configure file that checks OS before creating a Makevars file to specify compilation with openMP or not, using autoconf.
    \item See also: ARTP2, mvnfast (openMP in source files, but not OSX binary)
    \item TODO: Add note in DESCRIPTION file to inform Mac users about lack of openMP and possible solutions -- symbolically link openMP-enabled compilers, etc.
...
Agenda for May 22: Planning for tomorrow's simulation runs to test the RFP model again (TODO: Still need to rename it... Low priority).
...
\begin{itemize}
    \item Currently, Gauley still doesn't compile package correctly, but Newton does.
    \item Removed old read/writeRFP functions, fixed documentation around these areas of code, cleaned up testing folder and naming convention (.pa files renamed to standard .csv).
    \item Moved desc folder from RibModelFramework to RibModelDev.
\end{itemize}
