...
\experiment{RFP Structure}
\begin{itemize}
    \item From the last time I talked with Dr. Gilchrist, I mentioned to him the bugs I had with simulating a genome for the PA model, and he suggested it might have because we had changed the structure of the read-in function to account for per-position and additional categories, while the simulation, unedited, only took the sum RFP count. While looking over the simulation functions with Denizhan, I confirmed this was indeed the cause of the bugs.
...
\begin{itemize}
    ...
    \item This basically resulted in a hidden change to my local version of the labbook directory that I could not overwrite but would be detected with trying to merge to the current version of the labbook repo on git that Denizhan later updated to fix the symbolic link error.
    \item Thus, I had a merge conflict that could not be resolved; I fixed this by deleting and re-cloning the labbook repo, but I accidentally lost the notes from the past two days. I rewrote these notes and pushed them.
