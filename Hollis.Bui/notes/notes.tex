...
\begin{itemize}
    ...
    \item I'm still unsure of how to handle inheritance in sharing variables between just two models (PA, PANSE): the RFPCountColumn variable should be accessible in both, I think, instead of each different model defining the same thing.
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 2:00 - 3:00 -- Cleaned up changes from yesterday, pushed changes. 
    \item 3:00 - 5:00 -- Eventually fixed my IDE settings; I had overwritten my repository accidentally while comparing versions to find a working Simulate Genome function.
\end{itemize}

\begin{itemize}
    \item Pushed changes from yesterday (forgot to when I got home).
    \item General note to self from yesterday's discussion: default arguments in C++ are \textbf{never} passed into R. So I have to pass these arguments into R every time (or create default arguments in R functions).
    \item One idea re: sharing variables of PA, PANSE is to actually make PANSE inherit PA's functions and variables. This would make the virtual keywords in PA actually useful since it could be a full extension of PA. This would make the inheritancy Model -> PA -> PANSE.
    \item Discussed this with Denizhan. Perhaps once PANSE is completely finished and tested we can actually switch inheritance to PANSE -> PA, but for now it's the other way around for convenience.
    \item Spent an hour and a half fixing a problem with my IDE.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 1:30 - 2:00 -- Consolidated hours, pulled from repos and updated.
    \item 2:00 - 4:30 -- Reviewing files, notes; some discussion with Denizhan.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 10:30 - 1:00 -- Set up a testing branch for Rib Model Framework to debug simulateGenome() further.
    \item 2:30 - 3:00 -- Continued looking through notes. Minor style edits corrected (found them while looking through files debugging).
    \item 3:00 - 3:45 -- Talk with Dr. Gilchrist
    \item 3:45 - 5:00 -- Wrapped up notes and planning, headed home a bit early.
\end{itemize}

\experiment{PA-format Genome Simulation}
\begin{itemize}
    \item Last few days spent checking how to simulate a genome on the C++ side only, since that was what worked successfully last time.
    \item As of yet, bugging out -- not sure if it's because of input files or the function buggingout because of changes.
\end{itemize}

\experiment{Gilchrist Talk}
\begin{itemize}
    \item RPF (ribosome profile f...) is one of the names of the data formats we'll be working with. Keeping the name PA distinct from the file format, we should rename the read-in function for PA as \enquote{RFPData}.
    \item Make \enquote{RFPValue} more explicit as well in terms of what it means. Maybe \enquote{SumRFPCountForGene} or something.
    \item Hold off on simulating data for now; we can just compare Pop's data.
    \item Idea: Get median of Alpha and Lambda and use each as a good estimate of Alpha and Lambda values. Then we can use these to calculate expected waiting times as per rfp.model.pdf.
    \item Reminder: Inverse of these values are Pop's translation rates. We can just graph these data points together (x-y) to see if there's a roughly 1-1 correspondence.
    \item This should be much easier than what I was preparing to do originally; looks like my script was over-complicated.
    \item The phi data that is read-in is mostly for ROC, not for RFP-based data. Artifact of a messy RibModelDev directory.
    \item Still, after comparing rates, we should compare the Psi.g rates from our model with ROC's log phi values (ask Cedric where to get these values; we should already have them for Baker's Yeast).
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 9:00 - 12:00 -- Renamed functions as discussed yesterday.
    \item 3:00 - 6:00 -- Asked Cedric about the Phi data. He directed me to the 2015 paper's supplementary materials. Spent some time looking over the files while waiting for automated checking on renames to pass, fixing merge conflict.
\end{itemize}

\experiment{RFP Structure}
\begin{itemize}
    \item Now, read/writePA is renamed read/writeRFPData.
    \item For R-side scriping purposes, getRFPValue is now renamed, more accurately, getSumRFPCountForCodon.
\end{itemize}

\experiment{TODO}
\begin{itemize}
    \item When installing the package without building it (such as when running it in R), the cleanup file is not executed, resulting in a file and a folder that should be removed. May have to think of a way to run cleanup when installing the package.
    \item Denizhan's latest labnotebook commit created symbolic link errors when doing any git commands. TODO: Ask him about these links.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 2:30 - 5:00 -- Discussed with Denizhan about some questions he had about genome simulation, R scripting, and the symbolic link problem. Looked over misc. notes about R scripting myself to get read to write the script.
\end{itemize}

\experiment{RFP Structure}
\begin{itemize}
    \item From the last time I talked with Dr. Gilchrist, I mentioned to him the bugs I had with simulating a genome for the PA model, and he suggested it might have been because we had changed the structure of the read-in function to account for per-position and additional categories, while the simulation function, unedited, only calculated the sum RFP count. While looking over the simulation functions with Denizhan, I confirmed this was indeed the cause of the bugs.
    \item From rfp.model.pdf, simulating a genome does indeed only simulate the sum of the RFP count without worrying about position, so the current function is probably fine. Instead, I have to think of a way to extract this data and read it back in when running the MCMC.
    \item The same document also defines how to simulate a genome for PANSE, which does need positions -- Denizhan and I will have to figure out how to code this later. For now, I suggested to him to ask Dr. Gilchrist if he had any data on-hand to easily test PANSE's new calculations, similar to what I'm doing with Pop's data and PA.
    \item For fixing the simulation bug with the PA model, I am currently thinking of simply rewriting (aka copying) the old readRFP function from when it used the bugged calculations with number of codons. This function should be easier to change and use with just the sum of the RFP counts, rather than per-position codon counts.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 9:30 - 11:00 -- Fixed dumb git bug, rewrote notes from Monday.
    \item 11:00 - 1:00 -- Found and squashed some minor typing bugs and inconsistencies; typos. Tasked Denizhan with helping me check if the simulateGenome functions worked in R like they did in C++ (known issue).
    \item 2:00 - 5:00 -- Renamed RFPCategory, began re-adding simulated RFP data read/write functionality.
\end{itemize}

\experiment{Misc Notes}
\begin{itemize}
    \item I spent an hour and a half trying to fix a bug with github and merges. Apparently, Denizhan's symbolic links had a tangible consequence -- because I pulled a version that had the symbolic link error, git would complain continuously that there were too many layers of symbolic links.
    \item This basically resulted in a hidden change to my local version of the labbook directory that I could not overwrite but would be detected when trying to merge to the current version of the labbook repo on git that Denizhan later updated to fix the symbolic link error.
    \item Thus, I had a merge conflict that could not be resolved; I fixed this by deleting and re-cloning the labbook repo, but I accidentally lost the notes from the past two days from carelessly losing my copy buffer. I rewrote these notes and pushed them.
    \item On a different note, it seems like I need to run the pdflatex command twice on my .tex files in this repo to get a working table of contents. Not sure why it lists a error with finding a latex function definition when I run the command once, but doesn't on the second time.
\end{itemize}

\experiment{RFP Structure}
\begin{itemize}
    \item Decided to go ahead and re-add parts of the old read/writeRFP functions into the current code for simulated genomes.
    \item Planned implentation: The current writeRFPData function, when its optional simulated boolean is set to true, branches into a fairly different way of writing the RFP data, adapted from the old writeRFP function.
    \item Meanwhile, reuse the old read\sep RFP functon as read\sep Simulated\sep Genome\sep From\sep PA\sep Model.
    \item Also renamed RFPCategory to RFPCountColumn in the .cpp code.
    \item Left off the day with writeRFP(simulated) written, almost done with readSimulatedGenomeFromPAModel. Still need to test all this of course.
    \item Motivation for doing this is to get one step closer to having simulation re-fixed for Denizhan to then be able to modify for PANSE and then let him be busy writing that function.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 2:00 - 5:00 -- Looked over some stuff with Denizhan on simulating genomes, preparing him for the process to run models.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 1:00 - 4:00 -- Have hours recorded here for some reason in a notepad document. I think I worked on writing the read/write RFP functions? Will not put down hours just in case I was wrong. -- Hollis, July 10, 2017.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 9:30 - 1:40 -- Finished up most of read\sep Simulated\sep Genome\sep From\sep PA\sep Model and write\sep RFP (simulated). Still need to test them. Requires some design decisions; tasked to Denizhan.
    \item 1:40 - 3:30 -- Talk with Dr. Gilchrist.
    \item 3:30 - 4:30 -- Began looking at R script to apply changes.
\end{itemize}

\experiment{RFP Structure}
\begin{itemize}
    \item Again, the differences between simulated data and non-simulated data are annoying.
    \item In the PA Model, summed RFPs are generated without regard to position, so the current read\sep Simulated\sep Genome\sep From\sep PA\sep Model format cleanly gets the Gene\sep ID\sep ,\sep Codon\sep ,\sep Codon\sep \_\sep Count\sep ,\sep RFP\sep Count (for only a single category; intended).
    \item TODO is to make the header dynamic (so instead of RFPCount, it says long or short) as an RFPCount category. This will be the same single category specified on model creation.
    \item Bigger problem is ensuring (unit-testing) that the read and write of simulated data is correct. Probably easiest to compare the stored number of codons per codon is equal and their sumRFPCouns are equal. Still a fairly lengthy project and Denizhan was looking for some work (and this advances his own work with simulating PANSE later), so I described the problem to him as a potential TODO.
    \item Pushed a change where the functions are implemented, but untested. Looking at output files at a glance shows that they are working correctly, but need to test in practice with unit testing techniques.
\end{itemize}

\experiment{Talk with Dr. Gilchrist}
\begin{itemize}
    \item Unfortunately a lot of the instructions were missed. I need to keep the phi in mind, but my test run on Sunday just used the code leftover from before (mystery phi source from Jeremy's old files).
    \item TODO: use 009670-6.tsv from the 2015 Gilchrist Paper as a source for s.phi to be compared with Psi. Recall: When we initialize the genome and request files, Phi is one of the files requested in the script I hacked up.
    \item \textbf{Need} to be better about recording this information. Sunday's notes were lost as well during the great github disaster.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 1:30 - 3:00 -- Talked with Denizhan about his tasks, reviewed material from past few days.
    \item 4:00 - 6:00 -- Setup for R scripting. Moved files to this directory for more transparency. Folder named June2017PAModelPop, found in RunnableScriptsAndData.
\end{itemize}

\experiment{TODO}
\begin{itemize}
    \item The .tsv file may need to be processed. Check if R supports reading .tsv files, otherwise write a script to organize the data in a readable format.
    \item Ended the day still a bit confused on how to do some of the R scripting to get data columns. Continuing to read documentation tomorrow.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 10:30 - 12:00 -- Looked up R documentation for scripting the Pop data. Lots of discussion with Denizhan on how to proceed with testing simulated genome.
    \item 1:00 - 2:30 -- Talk with Dr. Gilchrist.
    \item 2:30 - 5:00 -- Sorted notes and began scripting to preprocess data for actual R scripting.
\end{itemize}

\experiment{R Scripting}
\begin{itemize}
    \item Found the command to read a table in .tsv format.
\end{itemize}

\experiment{Talk with Dr. Gilchrist}
\begin{itemize}
    \item Among other notes and TODOs, rename in PA/PANSEParameter initMutationSelectionCategories to initAlphaLambdaCategories.
    \item TODO: Ask Cedric if it'd be ok to rename mixtureDefinition's delM and delEta variables to something broader like category1 and category2.
    \item I should be using the initializeSynthesisRate (by list) function after using readPhiValues to read in the .tsv file.
    \item Unfortunately this function does not account for .tsv format (instead assumes .csv format). Will have to remove the handy R function to read .tsv and instead pre-process.
    \item TODO: instead of using the Pop data file I had before, just sample 500 genes, and maybe just a subset of the codons too (say, 20). More scripting but this has been written before.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 2:00 - 4:00 -- Worked from home, scripted a quick reordering of the .tsv file after recovering old scripts.
    \item 4:00 - 6:00 -- Recompiling the package leads to errors; suspect it's related to Denizhan's changes on Friday adding Cmake stuff. Spent time debugging.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 11:00 - 1:00 -- Reviewed Denizhan's changes from Friday and did some style changes and corrections.
    \item 1:45 - 2:10 -- Compiled hours for timesheet.
    \item 2:10 - 4:30 -- Talked with Denizhan some more about simulateGenome for the PAModel working for R; scripting for file processing.
\end{itemize}

\experiment{RFP Structure}
\begin{itemize}
    \item Renamed a helper function (testEqualityGenome) to testGenomeSimulatedPAEqualityHelper. I think that while this name is long, it's descriptive (and only used in unit testing anyway); I think we will also need a variant of this function when we test equality of PANSE.
    \item Asked Cedric about some of the naming decisions. A lot of it is based on inheritance from ROC Model, so the code works for now but with inherited names. TODO: Ask Dr. Gilchrist for ideas on renaming, if he thinks it's even necessary. Probably not?
\end{itemize}

\experiment{Misc. Package Maintenance}
\begin{itemize}
    \item Fixed some of the .Rbuildignore and .gitignore problems from Cmake (including extraneous files).
    \item While reviewing package contents, re-discovered the testPAModel.R function I wrote a while back for automated testing of the then-called RFPModel!
    \item This file had actually been disabled because Gauley had failed to generate the same results, so to ensure the package checks passed on all systems (Gauley was the only holdout), I had commented out the automated testing pass.
    \item It may be much simpler to re-enable this now that Gauley should work and let me see if the PAModel works according to known codon translation rates.
    \item Now going to spend some time updating this to new standards (such as adding in equality checks); scripting to change .csv file to PA format again.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 12:45 - 2:30 -- Looked over old testPAModel.R code; talked to Alex about how to include Phi information into the parameter object in R.
    \item 2:30 - 3:20 -- Talk with Dr. Gilchrist; cleared up difference between phi and psi and how it will affect the framework in the future, and reviewed current progress and TODOs.
    \item 3:20 - 4:50 -- Discussed some of the future changes to be made to the framework with Denizhan. Looked over the documentation to see how phi (psi) is handled when read in and initialized.
\end{itemize}

\experiment{R Scripting}
\begin{itemize}
    \item Shelving testPAModel.R stuff for now. I think it might be better to get the original R script working that Dr. Gilchrist requested.
    \item Bright side is that the same Perl scripting code used to create those files can be modified to create the 500-gene sample for the script.
    \item Alex also pointed to a simpler to way insert Phi data into the model: use Sphi\_init after all, which should work if I read it into a table for the Parameter initialization.
    \item Need to remember to \enquote{align} the phi values and which geneID it was related to with the same geneID order of the rfpCount file.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 12:45 - 1:45 -- Talked with Cedric about how to use Phi initialization; wrote up notes and push a change to save progress.
    \item 2:00 - 5:00 -- Spent time processing the Phi data via Perl scripting; time split between reading documentation and writing/testing scripting code results. Because the phi data included different geneIDs than used in Pop's data, I had to isolate only the valid geneIDs and get those phi values. Perl scripting is done and results can be found in Runnable\sep Scripts\sep And\sep Data\sep /\sep June\sep 2017\sep PA\sep Model\sep Pop\sep /\sep testing\sep In\sep /\sep gene\sep ID\sep Phi\sep Mean\sep .\sep csv.
\end{itemize}

\experiment{R Scripting}
\begin{itemize}
    \item Cedric pointed out that Sphi\_init is actually the standard deviation of Phi, which isn't what I should be trying to modify.
    \item Instead, there's a different variable in the initialization of the parameter object, initial.expression.values, that I can input the phi values into.
    \item I will be using the mean of the phi values in the .tsv file.
    \item TODO tomorrow is to now use this .csv file I created with just the second column of data as the phi initialization.
    \item Also need to get only a random subset of this data. The geneIDPhiMean.csv file has 2600 genes, missing about 40 from Pop's number of genes.
    \item Of course, once those random geneIDs are minimized, I'll also have to get the same subset for the rfpCount file.
    \item Only once I cut out most of these genes can I run my actual R script plotting data.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 2:00 - 5:00 -- Shorter day spent trying to script the Pop data, randomize it, etc. discussion with Denizhan about the Genome model again.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 12:00 - 2:00 -- Finally nailed down the bugs with randomizing data. Now have a file, PopPADataRand500.csv, which contains the subset of PopPAData that contains only gene data from a set of 500 genes (listed in randNames.out).
    \item 2:00 - 4:30 -- Also had to re-organize the randGeneIDPhiMean.csv file, which had originally randomized the geneIDPhiMean.csv file, but in a random order unlike the subset of PopPAData. Did this by simply alphabetically ordering both files keyed on GeneID.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 12:00 - 1:30 -- Discussion with Denizhan and Cedric; compiled Dr. Gilchrist's email from Friday into Latex for convenience.
    \item 1:30 - 5:30 -- Started writing, updating, testing the R script with the new files for initializing Phi and Pop's data.
\end{itemize}

\experiment{RFP Structure}
\begin{itemize}
    \item Cedric says that the std\_csp vector that we use for PA and PANSE model is deprecated; we should be using a 2-by-2 covariance matrix, similar to what ROC does. Far-flung TODO for now.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 9:00 - 10:00 -- Working from home, rebuilt packages and began MCMC with my R script on 1000 genes.
    \item 1:30 - 3:30 -- Finally remembered to push notes. The MCMC on 1000 sample, 500 genes completed successfully. Talked with Denizhan about how to run his own R script, and also updated README.md on confusing Perl scripts.
    \item 3:30 - 5:00 -- runPAModelScript2017.R seems to have most of the old code still working, but currently running into an issue where the Alpha and LambdaPrime values yield infinity. Will check more later.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 1:30 - 3:00 -- Continuing debugging of my R script.
    \item 3:00 - 5:00 -- Switched to looking at C++ code to see when things are changed.
    \item 5:00 - 6:00 -- Talked with Denizhan about possible TODOs for the project overall.
\end{itemize}

\experiment{R Scripting}
\begin{itemize}
    \item After running a couple of other 1000-sample, 500 gene runs, still having trouble seeing why both alpha and lambda values are zero.
    \item Going to start looking back into the C code to see if the calculations are off from weird input values.
    \item The initial.expression.values (whether I have Dr. Gilchrist's phi values or not initialized) do not affect how alpha/lambda go to infinity.
    \item Also cannot see the trace plots at all for these values.
    \item I can, however, access the synthesisRatePosteriorMean, so the model isn't completely broken.
\end{itemize}

\experiment{TODO}
\begin{itemize}
    \item I \textbf{need} to talk to Cedric for help on debugging this infinite return value on my codon-specific (Alpha and LambdaPrime) values. Finished day not finding a solution.
    \item Denizhan and I may also want to talk about the models' destructors and necessity of copy constructors (rule of three in c++).
    \item Mentioned to Denizhan a long-term goal is automated testing on validity of the models -- see testMCMCROC.R, an old file that currently checks just the logPosterior to see if it's accurate with a set seed and hardcoded values known to be true. Ideally we expand this further, and make a version for each model. Far-off idea.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 3:00 - 5:00 -- Working from home, made another notes file from some of the Gilchrist meeting notes.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 12:00 - 2:00 -- Helped Denizhan debug some of his issues with the R script. Pushed a change to the main repo after fixing merge conflict. 
    \item 2:00 - 4:00 -- Exposed setGroupList() to R; now we can isolate certain codons when running MCMC instead of manually changing scripts.
    \item 4:00 - 5:00 -- Gilchrist meeting.
    \item 5:00 - 6:15 -- Wrote up notes after meeting with Dr. Gilchrist.
\end{itemize}

\experiment{TODO}
\begin{itemize}
    \item Floats: There are some Float data types in C++ instead of Doubles. Cedric said they were there to save space since we would not need the precision when outputting data (but we do want this precision interally as Doubles). Need to commentate this and follow this same idea for any remaining variables to avoid confusion.
    \item Rule-Of-Three (C++) -- several comments everywhere in each .cpp file. Cedric says it's not a bad idea to have copy assignments/copy constructors and follow Rule of Three. Note that we mostly pass pointers to avoid the copy constructor (example: Parameter is a pointer). This does mean that Model needs a constructor to destroy the parameter object when it is destroyed.
    \item Note: According to /url{https://stackoverflow.com/questions/14184341/c-constructor-destructor-inheritance}, we don't need to reference Model.cpp's constructor/destructor in subclasses.
    \item TODO: Test framework still works after I try to delete Parameter in a given Model object upon calling the destructor. Do this AFTER the runs with Dr. Gilchrist.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 10:00 - 12:30 -- Working at home: Running the entire genome still yields infinity values for Alpha and Lambda Prime. Results are in the June2017PAModelPop directory under OutputFullDataPop. Spent a couple of hours looking at C++ code some more.
    \item 2:00 - 5:00 -- Started writing and compiling further notes and TODOs for the project after I leave for future researchers' benefit.
\end{itemize}

\experiment{R Scripting}
I am attempting to run my script with the full genome (~4600 genes) and 1000 samples.

\labday{July 23, 2017}

\experiment{Time Breakdown}
\begin{itemize}
    \item 11:45 - 1:00 -- Spent about an hour trying to get Gauley to remotely run my R script. Also getting infinity on that machine.
    \item 1:00 - 4:00 -- Further note-writing and compilation of TODOs.
\end{itemize}

\experiment{TODO}
\begin{itemize}
    \item Consider setting up Newton to run the R script. Gauley seemed a bit slow still.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 12:00 - 2:00 -- Spent some time explaining how my model testing is going with Denizhan. He suggested to run the script with just one gene this time, too. Began writing this version of the script.
    \item 2:00 - 5:00 -- Compiled lab hours, further discussion on thing to do with the PANSE model moving forward. Explained the conversion of phi to psi to Denizhan.
\end{itemize}

\experiment{R Scripting}
Now considering running a version of the script that does not include initialized psi (phi) values but with the full genome. Might also "fix" the infinity error.

\labday{July 25, 2017}

\experiment{Time Breakdown}
\begin{itemize}
    \item 1:00 - 3:00 -- Went over some time-saving techniques with Denizhan on workflow (Travis.ci setup, github sshing, ssh setup with remotes such as Gauley).
    \item 3:00 - 5:30 -- Even more attempts to debug the script. Created another subset of the PopPAData that uses only one gene (a single one) and tested it in R to no new results.
\end{itemize}

\experiment{R Scripting}
\begin{itemize}
    \item I'm beginning to suspect that the infinity output may be due to a non-input error.
    \item Besides the C++ code, I should check the R code to see if I'm even outputting "valid" function arguments or whatever. Maybe I am printing non-existent values which default to infinity?
    \item Denizhan also suggested I try running my script with just one gene from Pop's data to see if the error is caused over time. I tried this to no new results (1000 samples, no phi initialization).
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 9:00 - 11:00; 12:00 - 2:00 -- Slog of looking over results some more, this time based on a C++ run of the MCMC. Still results in finding infinity values.
    \item 2:00 - 5:00 -- Switched over to writing more notes, TODOs, and minor clarifications of the C++ code where needed.
\end{itemize}

\experiment{R Scripting}
\begin{itemize}
    \item In main.cpp, we can run a C++ -based run of the MCMC, getting the same results but lacking R-specific plotting functionality.
    \item Running this with the one-gene subset from Pop's data (and no phi initialization).
    \item Switched over to writing up more TODOs on the project looking ahead for Denizhan. Today I focused more on the C-side of the code. Pushed a couple of these changes.
\end{itemize}
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 12:00 - 2:40 -- Fixed a minor bug in my branch of the repo from forgetting to rename an R-specific variable that was affected by a C-specific variable rename. Misc. similar documenation and code cleanup.
    \item 2:40 - 5:00 -- Talk with Dr. Gilchrist on debugging, later attempts to debug.
    \item 5:00 - 6:00 -- In the midst of writing notes, discussed with Denizhan his progress on PA(NSE) and how his script works yet mine doesn't -- he's running PANSE with Psi=Phi so it should be equivalent to PA. Wrote notes, pushed labbook.
\end{itemize}

\experiment{R Scripting}
\begin{itemize}
    \item More thoughts on debugging:
    \item Denizhan suggested it might have been a recent change I made to my own branch (not pushed) where I cast doubles to floats, which might affect the conversion (note: we already do this conversion, just without explicit casting). Rerunning the script on one gene did not change anything.
    \item Denizhan also sent me his script on PANSE which did get Alpha and Lambda Prime values. Might be useful to use the same input since, if working correctly, PANSE = PA for now.
    \item  Dr. Gilchrist suggested I output the literal numeric trace for Alpha and Lambda to nail down the initial value and when it diverges to infinity. Both start at 1.0 for codon "AGT". Of course, since they diverge to infinity, this is why it's impossible to plot these traces.
\end{itemize}
