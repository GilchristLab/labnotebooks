...
\begin{itemize}
    ...
    \item In the PA Model, summed RFPs are generated without regard to position, so the current read\sep Simulated\sep Genome\sep From\sep PA\sep Model format cleanly gets the Gene\sep ID,Codon,Codon\sep \_\sep Count,RFP\sep Count (for only a single category; intended).
...
\experiment{R Scripting}
\begin{itemize}
    \item Shelving testPAModel.R stuff for now. I think it might be better to get the original script working that Dr. Gilchrist requested.
    \item Bright side is that the same scripting code used to create those files can be modified to create the 500-gene sample for the script.
