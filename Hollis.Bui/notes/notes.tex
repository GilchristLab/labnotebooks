...
\begin{itemize}
    ...
    \item Meanwhile, reuse the old readRFP functon as readSimulatedGenomeFromPAModel.
...
\experiment{Time Breakdown}
\begin{itemize}
    \item 9:30 - 1:40 -- Finished up most of readSimulatedGenomeFromPAModel and writeRFP (simulated). Still need to test them. Requires some design decisions; tasked to Denizhan.
...
\begin{itemize}
    ...
    \item In the PA Model, summed RFPs are generated without regard to position, so the current readSimulatedGenomeFromPAModel format cleanly gets the GeneID,Codon,Codon\_Count,RFPCount (for only a single category; intended).
...
\begin{itemize}
    ...
    \item TODO As well is to, instead of using the Pop data file I had before, just sample 500 genes, and maybe just a subset of the codons too (say, 20). More scripting but this has been written before.
\end{itemize}
